...
class Pose:
    num_kpts = 18 # original 18 pts + Hidden COM keypt
    kpt_names = ['nose', 'neck',
    ...
    last_id = -1
    color = [0, 224, 255]
    com_color_good = [0, 255, 0]
    com_color_bad = [0, 0, 255]
    c_pt_color = [232, 232, 104]
    bos_color = [0, 0, 0]

    def __init__(self, keypoints, confidence, C_pts, BOS):
        super().__init__()
    ...
    def draw(self, img):
        assert self.keypoints.shape == (Pose.num_kpts+1, 2) # change to +1 for COM
        BOS = self.BOS
        ...
        if self.keypoints[-1, 0] != -1: # final COM
            x_c, y_c = self.keypoints[-1]
            color = []
            if x_c < BOS[0][0] or x_c > BOS[1][0]:
                color = Pose.com_color_bad
            else:
                color = Pose.com_color_good
            cv2.circle(img, (int(x_c), int(y_c)), 10, color, -1)
        
        if self.C_pts.tolist() != []: # segment COM
            for x_cp, y_cp in self.C_pts:
                cv2.circle(img, (int(x_cp), int(y_cp)), 9, Pose.c_pt_color, -1)
            for x_b, y_b in BOS:
                cv2.line(img, (int(x_b), int(y_b)), (int(x_b), 0), Pose.bos_color, 4)
